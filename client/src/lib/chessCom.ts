...
interface ChessComArchiveGameResponse {
  url?: string;
  pgn?: string;
  end_time?: number;
  time_class?: string;
...
interface ChessComRecentGamesResponse {
  player: ChessComPlayerSummary;
  games: ChessComGameSummary[];
}
...
async function collectRecentGames(archiveUrls: string[], limit: number): Promise<ChessComGameSummary[]> {
  ...
  for (let index = archiveUrls.length - 1; index >= 0; index -= 1) {
    ...
    const archive = await fetchJson<ChessComArchiveResponse>(archiveUrl);
    const archiveGames = (archive.games ?? [])
      ...
      .filter(function filterGame(game): game is ChessComGameSummary {
        return game !== null;
      });
